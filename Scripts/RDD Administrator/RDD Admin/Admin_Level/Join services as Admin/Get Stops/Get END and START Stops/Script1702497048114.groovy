...
// Función para obtener una parada aleatoria excluyendo las paradas con nombre "Direccion destino"
def obtenerParadaAleatoria(listaParadas) {
    def paradasExcluidas = listaParadas.findAll { it.name == "Dirección destino" }
    def paradasFiltradas = listaParadas - paradasExcluidas

    return paradasFiltradas[(int) (Math.random() * paradasFiltradas.size())]
}
