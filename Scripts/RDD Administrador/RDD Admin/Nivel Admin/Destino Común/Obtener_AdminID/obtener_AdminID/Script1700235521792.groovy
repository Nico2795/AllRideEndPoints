...
def response = WS.sendRequest(findTestObject('RDD Admin/obtener_ListaAdmins'))
...
if (response.getStatusCode() == 200) {
	...
	def targetAdminName = "Prueba AdminNico"  // Reemplazar con el nombre específico

	// Buscamos el Superadmin por el nombre en la respuesta JSON
	def targetAdmin = jsonResponse.find { it.name == targetAdminName }

	// Verifica si se encontró el Superadmin
	if (targetAdmin) {
		// Obtener el nivel de administrador del Superadmin
		def adminLevel = targetAdmin.adminLevel
		...
		GlobalVariable.AdminId = targetAdmin._id
		GlobalVariable.assignState = estadoAsignado
		
		println("Ahora la variable global de superadmin es: " + GlobalVariable.AdminId)
	} else {
		// Si no se encontró, imprime un mensaje indicando que no se encontró el Superadmin
		println "No se encontró el Superadmin con el nombre $targetAdminName en la respuesta."
	}
} else {
